...
export default function CreatePassword() {
  ...
  const CONFIRM_PASSWORD_VALID = confirmPassword !== "" && confirmPassword === password;
  ...
  return (
    <div className="flex flex-col h-screen px-4">
      ...
      <form className="space-y-8">
        ...
        <div className="relative rounded-md border border-gray-300 p-3 shadow-sm focus-within:border-green-600 focus-within:ring-1 focus-within:ring-green">
          ...
          <input
            ...
            placeholder=""
          />
          <button onClick={() => setConfirmPasswordVisibility(!confirmPasswordVisibility)} type="button" className="absolute inset-y-0 right-0 flex py-1.5 pr-1.5">
      ...
      <div className="flex items-end h-full">
        <Link
          passHref
          href="/signup/create_username"
        >
        <a className={`${isValid ? 'bg-green-600' : 'cursor-pointer bg-coolblack-200'} inline-flex justify-center items-center lg:mb-10 mb-4 uppercase w-full rounded-lg border border-gray-300  px-6 py-4 text-md font-bold text-white shadow-sm `}>
          next
        </a>
        </Link>
      </div>
